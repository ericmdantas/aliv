...
module.exports = class Server extends EventEmitter {
  constructor(opts) {
    ...
    this.opts = {};
    this.alivrcOptions = {};
    this.opts.root = opts && opts.root ? opts.root : process.cwd();
    this.alivrcPath = path.join(this.opts.root, '.alivrc');
  ...
  _normalizeOpts() {
    this.rootWatchable = path.join(this.opts.root, this.opts.pathIndex);
  ...
  _initStatic() {
    ...
    this._app.use(express.static(this.opts.root, {
      index: false
    }));

    this._app.use(express.static(this.rootWatchable, {
      index: false
    }));
  ...
  _clientConnected() {
    let fsWatcher = chokidar.watch(this.opts.only, {
      ignored: this.opts.ignore, persistent: true
    });
