...
module.exports = class Server extends EventEmitter {
  ...
  _createsProxy() {
    if (this.opts.proxy) {
      for (var index = 0; index < this.opts.proxyOptions.length; index++) {
        var proxyOption = this.opts.proxyOptions[index];
        proxyOption.proxyWhen += /\*$/.test(this.opts.proxyWhen) ? '' : '*';
        this._proxyServers.push({
          proxyTarget: proxyOption.proxyTarget,
  ...
  _initProxy() {
    if (this.opts.proxy) {
      for (var index = 0; index < this.opts.proxyOptions.length; index++) {
        let proxyOption = this.opts.proxyOptions[index];
        let proxyServer = this._proxyServers.filter(p => p.proxyTarget === proxyOption.proxyTarget)[0].proxyServer;
