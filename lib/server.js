...
module.exports = class Server extends EventEmitter {
  constructor(config) {
    ...
    this._alivConfigFile = {}
    this._alivConfigJsFileName = 'aliv.config.js'
    this._alivrcFileName = '.alivrc'
  ...
  use(fn) {
    this._middlewares.push({
      index: this._middlewares.length,
      fn: fn
    })
  }
  ...
  reload() {
    return new Promise((resolve, reject) => {
      if (!this.opts.reloadDelay) {
        return resolve(this._reload())
      }

      setTimeout(() => {
        resolve(this._reload())
      }, this.opts.reloadDelay)
    })
  }  

  _runMiddlewares(middleware, done) {
    if (middleware) {
      middleware.fn(() => {
        this._runMiddlewares(this._middlewares[middleware.index+1], done)
      })
    } else {
      done()
    }
  ...
  _reload() {
    return new Promise((resolve, reject) => {
        this._runMiddlewares(this._middlewares[0], () => {
  ...
  _parseAlivConfigFile() {
    const pathWithFileAlivConfigJsFile = path.join(this.opts.root, this._alivConfigJsFileName)
    const pathWithFileAlivrcFile = path.join(this.opts.root, this._alivrcFileName)

    if (this._file.exists(pathWithFileAlivrcFile)) {
      this._alivConfigFile = JSON.parse(this._file.read(pathWithFileAlivrcFile))
      return
    }

    if (this._file.exists(pathWithFileAlivConfigJsFile)) {
      this._alivConfigFile = require(pathWithFileAlivConfigJsFile)
      return
    }
  }
  ...
  _parseCliOptions(config) {
    ...
    Object.assign(this.opts, this._availableOptions, this._alivConfigFile, config)
  }
  ...
  _readIndex() {
    ...
    if (!~this._$.html().indexOf('<base href="/"') && !this.opts.quiet) {
      this._logger.warn('Consider using <base href="/" /> in the <head></head> of your index.html so deep routes work correcly.')
    }
  }
  ...
  _serverAlive() {
    ...
    this._logger.log(type + ' server alive, running on : ' + url)
  }
