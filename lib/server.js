...
module.exports = class Server {
  ...
  _createWebSocketServer() {
      ...
      return new WS(_httpServer).wsServer;
  }
  ...
  _onWsConnected(ws) {
    ...
    fsWatcher.on('change', (path, ev) => {
      ...
      this._sendReloadEvent(ws, path, ev);
  ...
  start() {
    this._initStatic();
    this._createWebSocketServer().on('connection', (ws) => this._onWsConnected(ws));
    this._openBrowser();
  }
}
