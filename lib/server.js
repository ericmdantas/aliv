...
module.exports = class Server {
  ...
  _onClientConnection(cb) {
      this.ws.server.on('connection', (client) => {
        this.ws.socket = client;
        cb();
      });
  }

  _onWsConnected(opts) {
    let fsWatcher = chokidar.watch('.', {ignored: opts.ignore, persistent: true});

    fsWatcher.on('change', (path, ev) => {
      this.ws.sendReload();
      file.log('changed', path, opts);
      fsWatcher.close();
    });
  }
  ...
  start() {
    ...
    this._onClientConnection(() => this._onWsConnected(this.opts));
    this._openBrowser();
  }
}
