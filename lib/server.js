...
module.exports = class Server extends EventEmitter {
  ...
  _readIndex() {
    ...
    if (!~this._$.html().indexOf('<base href="/"') && !this.opts.quiet) {
      log.info(chalk.yellow('Consider using <base href="/" /> in the <head></head> of your index.html so deep routes work correcly.'))
    }
  }
  ...
  _serverAlive() {
    ...
    log.info(type + ' server alive, running on : ' + url)
  }
  ...
  _createServer() {
    ...
    if (this.opts.port <= 1024 || (this.opts.redirectHttpToHttps && this.opts.insecurePort <= 1024)) {
      ...
      log.info(chalk.yellow('You are using the port ' + port + ' which is below or equal to 1024' + "\n" + 'start this script with elevated privileges to avoid EACCESS errors'))
    }
    ...
    if (this.opts.redirectHttpToHttps) {
      log.info(chalk.green('will redirect http (' + this.opts.insecurePort + ') traffic to https (' + this.opts.port + ')'))
